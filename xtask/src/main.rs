...
//! Cargo xtasks for gnucash-toolbox.
...
fn check_packages(ctx: &mut Context) {
    ...
    action!(
        ctx,
        "Checking that the gnucash-toolbox Nix package builds properly",
        "nix build -L --no-link .#gnucash-toolbox",
    );
